...
static inline bool is_zygote_process(struct task_struct *t)
{
	...
	if (t->children.next
			&& t->children.next != (struct list_head *)&t->children.next) {
...
static inline bool is_key_process(struct task_struct *t)
{
	...
	if (t->pid == t->tgid) {
		...
		if (pgrp != NULL) {
			...
			if (taskp != NULL && (is_systemserver_process(taskp)
					      || is_zygote_process(taskp))) {
				return true;
			}
		}
	}
...
static inline bool is_critial_process(struct task_struct *t)
{
	if (t->group_leader && (!strcmp(t->group_leader->comm, "system_server")
				|| !strncmp(t->group_leader->comm, "Binder:", 7)
				|| is_zygote_process(t) || !strcmp(t->group_leader->comm, "surfaceflinger")
				|| !strcmp(t->group_leader->comm, "servicemanager"))) {
		if (t->pid == t->tgid) {
			return true;
